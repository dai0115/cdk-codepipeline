...
export class PipelineBackendStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const codeCommitRoleBackendArn = new cdk.CfnParameter(
      this,
      "codeCommitRoleBackendArn",
      {
        type: "String",
    ...
    const codeCommitRole = iam.Role.fromRoleArn(
      ...
      codeCommitRoleBackendArn.valueAsString,
      {
        mutable: false,
      }
    );
    ...
    // codePipelineを作成
    const slackChannelArn = cdk.Fn.importValue("SlackConfigurationArn"); //TOOD フロントエンドで作成したslackArnを取得
    const slackChannel =
      chatbot.SlackChannelConfiguration.fromSlackChannelConfigurationArn(
        this,
        `slackChannel-${resourceName}`,
        slackChannelArn
      );
  ...
  private createCodepipelies(
    resourceName: string,
    slackChannel: chatbot.ISlackChannelConfiguration,
    role: IRole,
    bucket: IBucket
  ...
  private createArtifactBucket(
    ...
    const artifactBucket = new s3.Bucket(
      ...
      {
        bucketName: `${resourceName}s3bucket`, // ユニークな命名のため必要なパラメータを全て含めている// ユニークな命名のため必要なパラメータを全て含めている
