...
export class PipelineFrontendStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const codeCommitRoleFrontendArn = new cdk.CfnParameter(
      this,
    ...
    const codeCommitRole = iam.Role.fromRoleArn(
      ...
      codeCommitRoleFrontendArn.valueAsString,
      {
        mutable: false,
      }
    );
    ...
    // outputとして出力し、backend側でインポートして利用する。
    // slackの通知先は共通であることを想定。
    new cdk.CfnOutput(this, "OutputSlackConfiguration", {
      value: slackChannel.slackChannelConfigurationArn,
      exportName: "`SlackConfigurationArn",
    });
